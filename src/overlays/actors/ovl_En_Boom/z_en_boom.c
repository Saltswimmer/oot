...
void EnBoom_Fly(EnBoom* this, GlobalContext* globalCtx) {
    ...
    if (target != NULL) {
        ...
        if ((target != (Actor*)player) && ((target->update == NULL) || (ABS(yawDiff) > 0x4000))) {
            ...
            Math_ApproxUpdateScaledS(&this->actor.world.rot.y, yawTarget, (s16)(ABS(yawDiff) * distXYZScale));
            Math_ApproxUpdateScaledS(&this->actor.world.rot.x, pitchTarget, (s16)(ABS(pitchDiff) * distXYZScale));
        }
    }
    ...
    if (DECR(this->returnTimer) == 0) {
        ...
        if (collided) {
            ...
            collided = func_8003DE84(&globalCtx->colCtx, &this->actor.prevPos, &this->actor.world.pos, &hitPoint,
