...
void ObjBombiwa_Break(ObjBombiwa* this, GlobalContext* globalCtx) {
    ...
    for (i = 0; i < ARRAY_COUNT(sEffectScales); i++) {
        pos.x = ((Math_Rand_ZeroOne() - 0.5f) * 10.0f) + this->actor.home.pos.x;
        pos.y = ((Math_Rand_ZeroOne() * 5.0f) + this->actor.home.pos.y) + 8.0f;
        pos.z = ((Math_Rand_ZeroOne() - 0.5f) * 10.0f) + this->actor.home.pos.z;
