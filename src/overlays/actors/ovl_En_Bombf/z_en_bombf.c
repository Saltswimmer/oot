...
void EnBombf_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == BOMBFLOWER_BODY) {
        ...
        thisx->gravity = -1.5f;
        Actor_ChangeType(globalCtx, &globalCtx->actorCtx, thisx, ACTORCAT_EXPLOSIVE);
...
void EnBombf_Update(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == BOMBFLOWER_BODY) {
        ...
        if ((thisx->speedXZ != 0.0f) && (thisx->bgCheckFlags & 8)) {
            ...
            if (ABS((s16)(thisx->wallPolyRot - thisx->world.rot.y)) > 0x4000) {
                if (1) {}
                thisx->world.rot.y = ((thisx->wallPolyRot - thisx->world.rot.y) + thisx->wallPolyRot) - 0x8000;
            }
