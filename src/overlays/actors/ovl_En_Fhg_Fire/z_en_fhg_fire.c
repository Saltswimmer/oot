...
void EnFhgFire_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == 0x32) {
        ...
        thisx->world.rot.y = Math_atan2f(tempf1, tempf3) * 10430.378f; // 65536/(2*M_PI)
        tempf0 = sqrtf(SQ(tempf1) + SQ(tempf3));
        thisx->world.rot.x = Math_atan2f(tempf2, tempf0) * 10430.378f; // 65536/(2*M_PI)
...
void func_80A10008(EnFhgFire* this, GlobalContext* globalCtx) {
    ...
    if (this->fireMode == 0) {
        ...
        for (i = 0; i < 2; i++) {
            ballPos.x = Math_Rand_CenteredFloat(20.0f) + this->actor.world.pos.x;
            ballPos.y = Math_Rand_CenteredFloat(20.0f) + this->actor.world.pos.y;
            ballPos.z = Math_Rand_CenteredFloat(20.0f) + this->actor.world.pos.z;
            ballAccel.y = -0.08f;
