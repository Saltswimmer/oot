...
void BgBdanObjects_Init(Actor* thisx, GlobalContext* globalCtx) {
    ...
    if (thisx->params == 2) {
        thisx->flags |= 0x30;
        globalCtx->colCtx.stat.colHeader->waterBoxes[7].unk_02 = thisx->world.pos.y;
...
void func_8086C1A0(BgBdanObjects* this, GlobalContext* globalCtx) {
    if (Math_SmoothScaleMaxMinF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y + 500.0f, 0.5f, 7.5f,
                                1.0f) < 0.1f) {
...
void func_8086C3D8(BgBdanObjects* this, GlobalContext* globalCtx) {
    ...
    if (Math_ApproxF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y + -70.0f,
...
void func_8086C6EC(BgBdanObjects* this, GlobalContext* globalCtx) {
    s32 cond = Math_ApproxUpdateScaledS(&this->dyna.actor.shape.rot.y, this->dyna.actor.home.rot.y, 0x200);

    if (Math_ApproxF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y + -125.0f, 3.0f)) {
        if (cond) {
...
void func_8086C7D0(BgBdanObjects* this, GlobalContext* globalCtx) {
    if (Math_SmoothScaleMaxMinF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y + 965.0f, 0.5f, 15.0f,
                                0.2f) < 0.01f) {
        Audio_PlayActorSound2(&this->dyna.actor, NA_SE_EV_BUYOSTAND_STOP_A);
        this->actionFunc = BgBdanObjects_DoNothing;
    } else {
...
void func_8086C9F0(BgBdanObjects* this, GlobalContext* globalCtx) {
    if (this->unk_16A == 0) {
        if (Math_ApproxF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y, 0.5f)) {
            Flags_UnsetSwitch(globalCtx, this->unk_168);
            this->actionFunc = func_8086C9A8;
        }
        func_8002F948(this, NA_SE_EV_WATER_LEVEL_DOWN - SFX_FLAG);
    } else {
        if (Math_ApproxF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y + 75.0f, 0.5f)) {
            this->actionFunc = func_8086CABC;
        }
        func_8002F948(this, NA_SE_EV_WATER_LEVEL_DOWN - SFX_FLAG);
    }
    globalCtx->colCtx.stat.colHeader->waterBoxes[7].unk_02 = this->dyna.actor.world.pos.y;
}
